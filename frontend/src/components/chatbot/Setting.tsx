...
type ChatBotSettingContextType = {
    ...
    setEnabledHintInstructionType: (key: HintInstructionTypes, enabled: boolean) => void,
}

const enabledHintInstructionTypesInitialValue = Object.keys(HintInstructions) as HintInstructionTypes[]
...
export const ChatBotSettingContextProvider: FC<{
    ...
    const [enabledHintInstructionTypes, setEnabledHintInstructionTypes] = useState<enabledHintInstructionTypesType>(enabledHintInstructionTypesInitialValue);

    const setEnabledHintInstructionType = (key: HintInstructionTypes, enabled: boolean) => {
    ...
    return (
        <ChatBotSettingContext.Provider value={{ enabledHintInstructionTypes, setEnabledHintInstructionTypes, setEnabledHintInstructionType }}>
            {children}
...
export const useChatBotSettingContext = () => {
    ...
    if (!context) {
        throw new Error('ChatBotSettingContext must be used within a ChatBotSettingContextProvider');
    }
    return context;
};

export const ChatBotSettingComponent: FC = () => {
    const { enabledHintInstructionTypes, setEnabledHintInstructionTypes } = useChatBotSettingContext();

    const empty = "";
    type emptyType = "";
    const [inputs, setInputs] = useState<Array<HintInstructionTypes | emptyType>>(enabledHintInstructionTypes);
                                ...
                                <Select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (!Object.keys(HintInstructions).includes(newValue)) {
                                            throw new Error(`value ${newValue} does not exist in HintInstructionTypes`)
                                        }
                                        setInputs((prev) => {
                                            return Array.from(new Set(prev.map((v, vi) => (
                                                vi == i ? (newValue) : (v)
                                            ))))
                                        })
                                    }}
                                ...
                                >
                                    {
                                        Object.keys(HintInstructions).map((v, vi) => {
                                            return <MenuItem key={v} value={v}>type {vi + 1}</MenuItem>
                                        })
                                    }
                                </Select>
                                <IconButton onClick={() => {
            ...
            <IconButton color="default" onClick={() => {
                setInputs((prev) => {
                    return Array.from(new Set(
                        [...prev, empty]
                    )) as Array<HintInstructionTypes | emptyType>;
                });
            }}>
                <AddCircle />
            </IconButton>
