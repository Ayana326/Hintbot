      ...
      if (serializedMessage) {
        try {
          ...
          for (let messageStored of messagesStored) {
            if (typeof messageStored.user !== "string") throw new Error("type mismatch");
            for (let messageStoredAImessage of messageStored.ai) {
              if (typeof messageStoredAImessage.hint !== "string" || !Object.keys(HintInstructions).includes(messageStoredAImessage.hint_type)) throw new Error("type mismatch");
            }
          }
        } catch (e) {
          //localStorageのデータを削除
      ...
      <div className="w-full h-full">
        <div className="border rounded-md relative h-full">
          ...
          <div className="text-sending absolute bottom-1 right-0 left-0 m-auto w-11/12">
            <MessageBox problem={problem} messages={messages} setMessages={setMessages} />
          </div>
        </div>
      </div>
    );
